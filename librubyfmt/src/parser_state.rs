...
use crate::render_targets::{
	BaseQueue, Breakable, BreakableCallChainEntry, BreakableEntry,
};
...
impl<'src> ParserState<'src> {
		...
		F: FnOnce(&mut ParserState<'src>),
	{
		...
		next_ps.inside_squiggly_heredoc =
			kind.is_squiggly() || self.inside_squiggly_heredoc;
		...
		F: FnOnce(&mut ParserState<'src>),
	{
		...
		self.push_target(ConcreteLineTokenAndTargets::BreakableEntry(
			insert_be,
		));
	}
	...
	pub(crate) fn inline_breakable_of<F>(
		&mut self,
		...
		F: FnOnce(&mut ParserState<'src>),
	{
		...
		let insert_bcce = self
			...
			.expect(
				"This should be the BreakableCallChainEntry we just pushed",
			);
	...
	pub(crate) fn with_formatting_context<F>(
		&mut self,
	...
	pub(crate) fn on_line(&mut self, line_number: LineNumber) {
		...
		if let Some((comments, last_comment_line)) =
			self.comments_hash.extract_comments_to_line(
	...
	pub(crate) fn emit_string_content(&mut self, s: impl Into<Cow<'src, str>>) {
		...
		self.push_concrete_token(ConcreteLineToken::LTStringContent {
			content,
		});
	}
	...
	pub(crate) fn wind_dumping_comments_until_line(
		&mut self,
	...
	pub(crate) fn wind_dumping_comments_until_offset(
		&mut self,
		source_offset: SourceOffset,
	) {
		self.wind_dumping_comments_until_line(
			self.get_line_number_for_offset(source_offset),
		)
	}

	pub(crate) fn wind_dumping_comments(
		&mut self,
		maybe_max_line_number: Option<LineNumber>,
	) {
		...
		let should_iter = |ps: &ParserState, ln| {
			...
			ps.comments_hash.still_in_file(ln + 1)
				&& (ps.comments_hash.has_line(ln + 1)
					|| ps.comments_hash.is_empty_line(ln + 1))
	...
	pub(crate) fn hash_type_from_formatting_context(
		&self,
	) -> Option<&HashType> {
		self.formatting_context
			.iter()
	...
	pub(crate) fn get_line_number_for_offset(
		&self,
		source_offset: SourceOffset,
	) -> LineNumber {
	...
	pub(crate) fn render_heredocs(&mut self, skip: bool) {
		...
		for next_heredoc in heredocs {
			...
			let emit_as_raw =
				self.inside_squiggly_heredoc && !kind.is_squiggly();

			if !string_contents.is_empty() {
				if emit_as_raw {
					self.push_concrete_token(
	...
	pub(crate) fn emit_conditional_keyword(&mut self, contents: &'static str) {
		self.push_concrete_token(ConcreteLineToken::ConditionalKeyword {
			contents,
		});
	}
}
...
impl<'src> ParserState<'src> {
	...
	pub(crate) fn consume_to_render_queue(
		self,
	) -> Vec<ConcreteLineTokenAndTargets<'src>> {
		self.render_queue.into_tokens()
	}

	pub(crate) fn gather_heredocs(
		&mut self,
	) -> Option<Vec<HeredocString<'src>>> {
	...
	pub(crate) fn render_to_segments(self) -> Vec<HeredocSegment> {
		...
		fn flush_normal(
			current: &mut String,
			segments: &mut Vec<HeredocSegment>,
		) {
			if !current.is_empty() {
...
pub fn line_difference_requires_newline(
	to_line: LineNumber,
