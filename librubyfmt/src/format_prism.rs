...
pub fn format_node<'src>(ps: &mut ParserState<'src>, node: prism::Node<'src>) {
	...
	match node {
		...
		Node::AlternationPatternNode { .. } => format_alternation_pattern_node(
			ps,
			node.as_alternation_pattern_node().unwrap(),
		),
		Node::AndNode { .. } => {
			format_and_node(ps, node.as_and_node().unwrap())
		}
		Node::ArgumentsNode { .. } => {
			format_arguments_node(ps, node.as_arguments_node().unwrap())
		}
		Node::ArrayNode { .. } => {
			format_array_node(ps, node.as_array_node().unwrap())
		}
		Node::ArrayPatternNode { .. } => {
			format_array_pattern_node(ps, node.as_array_pattern_node().unwrap())
		}
		Node::AssocNode { .. } => {
			format_assoc_node(ps, node.as_assoc_node().unwrap())
		}
		Node::AssocSplatNode { .. } => {
			format_assoc_splat_node(ps, node.as_assoc_splat_node().unwrap())
		}
		Node::BackReferenceReadNode { .. } => format_back_reference_read_node(
			ps,
			node.as_back_reference_read_node().unwrap(),
		),
		Node::BeginNode { .. } => {
			format_begin_node(ps, node.as_begin_node().unwrap())
		}
		Node::BlockArgumentNode { .. } => format_block_argument_node(
			ps,
			node.as_block_argument_node().unwrap(),
		),
		Node::BlockLocalVariableNode { .. } => {
			format_block_local_variable_node(
				ps,
				node.as_block_local_variable_node().unwrap(),
			)
		}
		Node::BlockNode { .. } => {
			format_block_node(ps, node.as_block_node().unwrap())
		}
		Node::BlockParameterNode { .. } => format_block_parameter_node(
			ps,
			node.as_block_parameter_node().unwrap(),
		),
		Node::BlockParametersNode { .. } => format_block_parameters_node(
			ps,
			node.as_block_parameters_node().unwrap(),
		),
		Node::BreakNode { .. } => {
			format_break_node(ps, node.as_break_node().unwrap())
		}
		Node::CallAndWriteNode { .. } => format_call_and_write_node(
			ps,
		...
		Node::CallNode { .. } => {
			...
			let has_block =
				call_node.block().and_then(|b| b.as_block_node()).is_some();
		...
		Node::CallOperatorWriteNode { .. } => format_call_operator_write_node(
			ps,
		...
		Node::CapturePatternNode { .. } => format_capture_pattern_node(
			ps,
		...
		Node::ConstantTargetNode { .. } => format_constant_target_node(
			ps,
			node.as_constant_target_node().unwrap(),
		),
		Node::ConstantWriteNode { .. } => format_constant_write_node(
			ps,
			node.as_constant_write_node().unwrap(),
		),
		Node::DefNode { .. } => {
			format_def_node(ps, node.as_def_node().unwrap())
		}
		Node::DefinedNode { .. } => {
			format_defined_node(ps, node.as_defined_node().unwrap())
		}
		Node::ElseNode { .. } => {
			format_else_node(ps, node.as_else_node().unwrap())
		}
		Node::EmbeddedStatementsNode { .. } => format_embedded_statements_node(
			ps,
			node.as_embedded_statements_node().unwrap(),
		),
		Node::EmbeddedVariableNode { .. } => format_embedded_variable_node(
			ps,
			node.as_embedded_variable_node().unwrap(),
		),
		Node::EnsureNode { .. } => {
			format_ensure_node(ps, node.as_ensure_node().unwrap())
		}
		Node::FalseNode { .. } => {
			format_false_node(ps, node.as_false_node().unwrap())
		}
		Node::FindPatternNode { .. } => {
			format_find_pattern_node(ps, node.as_find_pattern_node().unwrap())
		}
		Node::FlipFlopNode { .. } => {
			format_flip_flop_node(ps, node.as_flip_flop_node().unwrap())
		}
		Node::FloatNode { .. } => {
			format_float_node(ps, node.as_float_node().unwrap())
		}
		Node::ForNode { .. } => {
			format_for_node(ps, node.as_for_node().unwrap())
		}
		Node::ForwardingArgumentsNode { .. } => {
			format_forwarding_arguments_node(
				ps,
				node.as_forwarding_arguments_node().unwrap(),
			)
		}
		Node::ForwardingParameterNode { .. } => {
			format_forwarding_parameter_node(
				ps,
				node.as_forwarding_parameter_node().unwrap(),
			)
		}
		Node::ForwardingSuperNode { .. } => format_forwarding_super_node(
			ps,
			node.as_forwarding_super_node().unwrap(),
		),
		Node::GlobalVariableAndWriteNode { .. } => {
			format_global_variable_and_write_node(
				ps,
				node.as_global_variable_and_write_node().unwrap(),
			)
		}
		Node::GlobalVariableOperatorWriteNode { .. } => {
			format_global_variable_operator_write_node(
				ps,
				node.as_global_variable_operator_write_node().unwrap(),
			)
		}
		Node::GlobalVariableOrWriteNode { .. } => {
			format_global_variable_or_write_node(
				ps,
				node.as_global_variable_or_write_node().unwrap(),
			)
		}
		Node::GlobalVariableReadNode { .. } => {
			format_global_variable_read_node(
				ps,
				node.as_global_variable_read_node().unwrap(),
			)
		}
		Node::GlobalVariableTargetNode { .. } => {
			format_global_variable_target_node(
				ps,
				node.as_global_variable_target_node().unwrap(),
			)
		}
		Node::GlobalVariableWriteNode { .. } => {
			format_global_variable_write_node(
				ps,
				node.as_global_variable_write_node().unwrap(),
			)
		}
		Node::HashNode { .. } => {
			format_hash_node(ps, node.as_hash_node().unwrap())
		}
		...
		Node::ImaginaryNode { .. } => {
			format_imaginary_node(ps, node.as_imaginary_node().unwrap())
		}
		...
		Node::IndexAndWriteNode { .. } => format_index_and_write_node(
			ps,
			node.as_index_and_write_node().unwrap(),
		),
		Node::IndexOperatorWriteNode { .. } => {
			format_index_operator_write_node(
				ps,
				node.as_index_operator_write_node().unwrap(),
			)
		}
		Node::IndexOrWriteNode { .. } => format_index_or_write_node(
			ps,
		...
		Node::InstanceVariableAndWriteNode { .. } => {
			format_instance_variable_and_write_node(
				ps,
		...
		Node::InterpolatedStringNode { .. } => format_interpolated_string_node(
			ps,
			node.as_interpolated_string_node().unwrap(),
		),
		Node::InterpolatedSymbolNode { .. } => format_interpolated_symbol_node(
			ps,
			node.as_interpolated_symbol_node().unwrap(),
		),
		Node::InterpolatedXStringNode { .. } => {
			format_interpolated_x_string_node(
				ps,
				node.as_interpolated_x_string_node().unwrap(),
			)
		}
		Node::ItLocalVariableReadNode { .. } => {
			format_it_local_variable_read_node(
				ps,
		...
		Node::KeywordRestParameterNode { .. } => {
			format_keyword_rest_parameter_node(
				ps,
				node.as_keyword_rest_parameter_node().unwrap(),
			)
		}
		Node::LambdaNode { .. } => {
			format_lambda_node(ps, node.as_lambda_node().unwrap())
		}
		Node::LocalVariableAndWriteNode { .. } => {
			format_local_variable_and_write_node(
				ps,
				node.as_local_variable_and_write_node().unwrap(),
			)
		}
		Node::LocalVariableOperatorWriteNode { .. } => {
			format_local_variable_operator_write_node(
				ps,
				node.as_local_variable_operator_write_node().unwrap(),
			)
		}
		Node::LocalVariableOrWriteNode { .. } => {
			format_local_variable_or_write_node(
				ps,
				node.as_local_variable_or_write_node().unwrap(),
			)
		}
		Node::LocalVariableReadNode { .. } => format_local_variable_read_node(
			ps,
			node.as_local_variable_read_node().unwrap(),
		),
		Node::LocalVariableTargetNode { .. } => {
			format_local_variable_target_node(
				ps,
				node.as_local_variable_target_node().unwrap(),
			)
		}
		Node::LocalVariableWriteNode { .. } => {
			format_local_variable_write_node(
				ps,
				node.as_local_variable_write_node().unwrap(),
			)
		}
		Node::MatchLastLineNode { .. } => format_match_last_line_node(
			ps,
			node.as_match_last_line_node().unwrap(),
		),
		Node::MatchPredicateNode { .. } => format_match_predicate_node(
			ps,
			node.as_match_predicate_node().unwrap(),
		),
		Node::MatchRequiredNode { .. } => format_match_required_node(
			ps,
		...
		Node::ModuleNode { .. } => {
			format_module_node(ps, node.as_module_node().unwrap())
		}
		...
		Node::PinnedExpressionNode { .. } => format_pinned_expression_node(
			ps,
			node.as_pinned_expression_node().unwrap(),
		),
		Node::PinnedVariableNode { .. } => format_pinned_variable_node(
			ps,
			node.as_pinned_variable_node().unwrap(),
		),
		Node::PostExecutionNode { .. } => format_post_execution_node(
			ps,
		...
		Node::ProgramNode { .. } => {
			format_program(ps, node.as_program_node().unwrap(), None)
		}
		Node::RangeNode { .. } => {
			format_range_node(ps, node.as_range_node().unwrap())
		}
		Node::RationalNode { .. } => {
			format_rational_node(ps, node.as_rational_node().unwrap())
		}
		Node::RedoNode { .. } => format_redo_node(ps),
		Node::RegularExpressionNode { .. } => format_regular_expression_node(
			ps,
			node.as_regular_expression_node().unwrap(),
		),
		Node::RequiredKeywordParameterNode { .. } => {
			format_required_keyword_parameter_node(
				ps,
				node.as_required_keyword_parameter_node().unwrap(),
			)
		}
		Node::RequiredParameterNode { .. } => format_required_parameter_node(
			ps,
			node.as_required_parameter_node().unwrap(),
		),
		Node::RescueModifierNode { .. } => format_rescue_modifier_node(
			ps,
			node.as_rescue_modifier_node().unwrap(),
		),
		Node::RescueNode { .. } => {
			format_rescue_node(ps, node.as_rescue_node().unwrap())
		}
		...
		Node::ReturnNode { .. } => {
			format_return_node(ps, node.as_return_node().unwrap())
		}
		Node::SelfNode { .. } => format_self_node(ps),
		Node::ShareableConstantNode { .. } => format_shareable_constant_node(
			ps,
			node.as_shareable_constant_node().unwrap(),
		),
		Node::SingletonClassNode { .. } => format_singleton_class_node(
			ps,
			node.as_singleton_class_node().unwrap(),
		),
		Node::SourceEncodingNode { .. } => format_source_encoding_node(
			ps,
		...
		Node::SplatNode { .. } => {
			format_splat_node(ps, node.as_splat_node().unwrap())
		}
		Node::StatementsNode { .. } => {
			format_statements(ps, node.as_statements_node().unwrap())
		}
		Node::StringNode { .. } => {
			format_string_node(ps, node.as_string_node().unwrap())
		}
		Node::SuperNode { .. } => {
			format_super_node(ps, node.as_super_node().unwrap())
		}
		Node::SymbolNode { .. } => {
			format_symbol_node(ps, node.as_symbol_node().unwrap())
		}
		Node::TrueNode { .. } => {
			format_true_node(ps, node.as_true_node().unwrap())
		}
		Node::UndefNode { .. } => {
			format_undef_node(ps, node.as_undef_node().unwrap())
		}
		Node::UnlessNode { .. } => {
			format_unless_node(ps, node.as_unless_node().unwrap())
		}
		Node::UntilNode { .. } => {
			format_until_node(ps, node.as_until_node().unwrap())
		}
		Node::WhenNode { .. } => {
			format_when_node(ps, node.as_when_node().unwrap())
		}
		Node::WhileNode { .. } => {
			format_while_node(ps, node.as_while_node().unwrap())
		}
		Node::XStringNode { .. } => {
			format_x_string_node(ps, node.as_x_string_node().unwrap())
		}
		Node::YieldNode { .. } => {
			format_yield_node(ps, node.as_yield_node().unwrap())
		}
	}
...
fn format_and_node<'src>(
	ps: &mut ParserState<'src>,
	and_node: prism::AndNode<'src>,
) {
...
fn format_begin_node<'src>(
	ps: &mut ParserState<'src>,
	begin_node: prism::BeginNode<'src>,
) {
...
fn format_break_node<'src>(
	ps: &mut ParserState<'src>,
	break_node: prism::BreakNode<'src>,
) {
	ps.emit_ident("break");
...
fn format_case_node<'src>(
	ps: &mut ParserState<'src>,
	case_node: prism::CaseNode<'src>,
) {
	ps.emit_case_keyword();
...
fn format_string_node<'src>(
	ps: &mut ParserState<'src>,
	string_node: prism::StringNode<'src>,
) {
	...
	if is_heredoc {
		format_heredoc(
			...
			opener.expect(
				"Heredocs must have an opening loc for the opening tag (<<FOO etc.)",
			),
		);
		return;
	}
	...
	ps.with_start_of_line(false, |ps| {
		...
		let in_escaped_context =
			is_heredoc || opener.map(|s| s.starts_with("\"")).unwrap_or(true);
		...
		ps.wind_dumping_comments_until_offset(
			string_node.content_loc().end_offset(),
		);
...
fn format_interpolated_string_node<'src>(
	...
	ps.with_start_of_line(false, |ps| {
		...
		for (i, part) in interpolated_string_node.parts().iter().enumerate() {
			...
			let indent_for_consecutive_strings =
				is_backslash_string_interpolation && i > 0;
...
impl<'src> HeredocNodeType<'src> {
	...
	fn closing_loc(&self) -> prism::Location<'src> {
		match self {
			HeredocNodeType::Plain(string_node) => {
				string_node.closing_loc().expect(
					"This is a heredoc, it must have a loc for the closing tag",
				)
			}
			HeredocNodeType::Interpolated(interpolated_string_node) => {
				interpolated_string_node.closing_loc().expect(
					"This is a heredoc, it must have a loc for the closing tag",
				)
			}
		}
...
fn maybe_render_heredocs_in_string<'src: 'a, 'a>(
	ps: &mut ParserState<'src>,
	peekable: &mut std::iter::Peekable<
		impl Iterator<Item = &'a prism::Node<'src>>,
	>,
) {
		...
		match part {
			prism::Node::StringNode { .. } => {
					...
					if common_indent > 0 {
						raw.split('\n')
							...
							.map(|(line_idx, line)| {
								...
								let should_strip =
									(line_idx > 0 || prev_ended_with_newline)
										&& !line.is_empty() && line.len()
										>= common_indent;
								if should_strip {
									&line[common_indent..]
								} else {
				...
				prev_ended_with_newline =
					if rendered_heredocs && contents.is_empty() {
						true
					} else {
						contents.ends_with('\n')
					};
			...
			prism::Node::EmbeddedVariableNode { .. } => {
				format_embedded_variable_node(
					ps,
					part.as_embedded_variable_node().unwrap(),
				);
				prev_ended_with_newline = false;
			}
...
fn format_ensure_node<'src>(
	ps: &mut ParserState<'src>,
	ensure_node: prism::EnsureNode<'src>,
) {
	// Double check that these offsets are correct, since begin/rescue/ensure/else
	// aren't always handled with `format_node`, which usually handles this
...
fn format_false_node<'src>(
	ps: &mut ParserState<'src>,
	false_node: prism::FalseNode<'src>,
) {
	...
	ps.with_start_of_line(false, |ps| {
		ps.new_block(|ps| {
			ps.breakable_of(BreakableDelims::for_array(), |ps| {
				...
				let requireds_end_offset =
					requireds.last().unwrap().location().end_offset();
...
fn format_class_node<'src>(
	ps: &mut ParserState<'src>,
	class_node: prism::ClassNode<'src>,
) {
	ps.emit_class_keyword();
	ps.emit_space();
	ps.with_start_of_line(false, |ps| {
		format_node(ps, class_node.constant_path())
	});
...
fn format_module_node<'src>(
	ps: &mut ParserState<'src>,
	module_node: prism::ModuleNode<'src>,
) {
	ps.emit_module_keyword();
	ps.emit_space();
	ps.with_start_of_line(false, |ps| {
		format_node(ps, module_node.constant_path())
	});
...
fn format_def_node<'src>(
	ps: &mut ParserState<'src>,
	def_node: prism::DefNode<'src>,
) {
	ps.emit_def_keyword();
	ps.emit_space();
...
fn format_defined_node<'src>(
	ps: &mut ParserState<'src>,
	defined_node: prism::DefinedNode<'src>,
) {
	ps.with_start_of_line(false, |ps| {
...
fn format_else_node<'src>(
	ps: &mut ParserState<'src>,
	else_node: prism::ElseNode<'src>,
) {
	// Double check that these offsets are correct, since begin/rescue/ensure/else
	// aren't always handled with `format_node`, which usually handles this
	...
	let keyword_len =
		else_keyword_loc.end_offset() - else_keyword_loc.start_offset();
	if keyword_len == 4 {
		ps.emit_else();
...
fn format_parameters_node<'src>(
	ps: &mut ParserState<'src>,
	params: prism::ParametersNode<'src>,
) {
	...
	//        |
	//    requireds
	let formats: &[for<'a> fn(
		&mut ParserState<'a>,
		prism::ParametersNode<'a>,
	)] = &[
		fmt_requireds,
		fmt_optionals,
		fmt_rest,
	...
	fn fmt_requireds<'a>(
		ps: &mut ParserState<'a>,
		params: prism::ParametersNode<'a>,
	) {
...
pub static OPTIONALLY_PARENTHESIZED_METHODS: [&str; 3] =
...
fn use_parens_for_call_node<'src>(
		...
		.map(|args| {
			!(args.arguments().is_empty()
				|| (args.arguments().len() == 1
					&& is_empty_parentheses_node(
						&args.arguments().first().unwrap(),
					)))
		})
		.unwrap_or(false);
	...
	if method_name.starts_with("attr_")
		&& context == FormattingContext::ClassOrModule
	{
...
fn format_call_node<'src>(
	...
	let is_dot_call =
		method_name == "call" && call_node.message_loc().is_none(); // e.g. `a.()`
	...
	if skip_receiver || call_node.receiver().is_none() {
			...
			&& !has_only_empty_paren_arg
		{
			...
			if arguments.arguments().len() == 1
				&& let Some(def_node) =
					arguments.arguments().first().unwrap().as_def_node()
			{
				ps.emit_space();
				...
				let last_arg = arguments.arguments().last().expect(
				...
				let maybe_closing_line =
					call_node.closing_loc().map(|closing_loc| {
				...
				ps.with_start_of_line(false, |ps| {
					ps.breakable_of(delims, |ps| {
						...
						if let Some(block_argument_node) =
							call_node.block().and_then(|block_node| {
								block_node.as_block_argument_node()
							}) {
							if has_arguments {
								ps.emit_comma();
								ps.emit_soft_newline();
						...
						if let Some(closing_line) = maybe_closing_line {
							ps.wind_dumping_comments_until_line(
								closing_line - 1,
							);
						}
					});
		...
		if let Some(block) = call_node.block() {
			...
			} else if call_node.arguments().is_none()
				&& block.as_block_argument_node().is_some()
			{
				ps.breakable_of(BreakableDelims::for_method_call(), |ps| {
					ps.emit_soft_indent();
					format_block_argument_node(
						ps,
						block.as_block_argument_node().unwrap(),
					);
				});
			}
		}
...
fn format_infix_operator<'src>(
	...
	ps.with_formatting_context(FormattingContext::Binary, |ps| {
		ps.with_start_of_line(false, |ps| {
			...
			//   baz
			// ```
			if let Some((inner_left, inner_op, inner_right)) =
				as_binary_op(&left)
			{
...
fn format_call_chain<'src>(
	ps: &mut ParserState<'src>,
	call_node: ruby_prism::CallNode<'src>,
) {
	ps.with_start_of_line(false, |ps| {
...
fn format_call_chain_segments<'src>(
	...
	if let Some(current) = segments.pop() {
		...
		let trailing_attr_write_value =
			chain_elements.last().and_then(|last| {
		...
		let is_user_multilined =
			call_chain_elements_are_user_multilined(ps, &chain_elements);
...
fn extract_trailing_arefs<'src>(
	...
	if has_dot_calls {
		while let Some(last_node) = elements.pop_if(|last_node| {
			last_node.as_call_node().is_some_and(|call_node| {
				call_node.call_operator_loc().is_none()
			})
		}) {
...
fn format_call_body<'src>(
	...
	{
		let call_operator =
			attr_write.call_operator_loc().map(|loc| loc_to_str(loc));
		...
		ps.with_start_of_line(false, |ps| {
			...
			for (idx, element) in call_chain_elements.iter().enumerate() {
				...
				let call_operator =
					element.call_operator_loc().map(|loc| loc_to_str(loc));
				if let Some(call_operator) = call_operator {
					if call_operator != "::" {
						ps.emit_collapsing_newline();
						ps.emit_soft_indent();
				...
				let skip_value = is_final_call
					&& skip_final_attr_write_value
					&& element.is_attribute_write();
...
fn call_chain_elements_are_user_multilined<'src>(
	...
	if call_chain_elements.len() > 1 {
		...
		if is_literal_expression {
			...
			let first_call_start_line = ps.get_line_number_for_offset(
				start_loc_for_call_node_in_chain(
					&call_chain_elements[1].as_call_node().unwrap(),
				),
			);
			let leading_expr_end_line = ps.get_line_number_for_offset(
			...
			let has_comment_between_expression_and_call = ps
				.has_comments_in_line(
					leading_expr_end_line + 1,
					first_call_start_line + 1,
				);
	...
	if first_is_not_call && second_is_aref {
		...
		while call_chain_elements.len() > 1
			&& let Some(call_node) =
				call_chain_elements.first().and_then(|n| n.as_call_node())
		{
	...
	let start_line = {
		...
		if let Some(call_node) = start_node.as_call_node() {
			ps.get_line_number_for_offset(start_loc_for_call_node_in_chain(
				&call_node,
			))
		} else {
...
fn start_loc_for_call_node_in_chain(
	call_chain_element: &prism::CallNode<'_>,
) -> usize {
	call_chain_element
        .message_loc()
        .unwrap_or_else(|| {
...
fn format_call_and_write_node<'src>(
	...
	ps.with_start_of_line(false, |ps| {
		format_node(ps, call_and_write_node.value())
	});
}
...
fn format_call_operator_write_node<'src>(
	...
	if let Some(call_operator_loc) =
		call_operator_write_node.call_operator_loc()
	{
		ps.emit_ident(loc_to_str(call_operator_loc));
	}
...
fn format_call_or_write_node<'src>(
	...
	ps.with_start_of_line(false, |ps| {
		format_node(ps, call_or_write_node.value())
	});
}
	...
	if is_single_quoted {
		...
		if let Some(value_loc) = symbol_node.value_loc() {
			...
			let escaped = crate::string_escape::single_to_double_quoted(
				content,
				opener.expect(
					"We must have an opener to know we're single-quoted",
				),
				closer.expect(
					"We must have a closer when wrapped in single quotes",
				),
			);
			ps.emit_string_content(escaped);
		}
...
fn format_assoc_node<'src>(
	ps: &mut ParserState<'src>,
	assoc_node: prism::AssocNode<'src>,
) {
	let as_symbol =
		if let Some(hash_type) = ps.hash_type_from_formatting_context() {
...
fn format_block_node<'src>(
	ps: &mut ParserState<'src>,
	block_node: prism::BlockNode<'src>,
) {
		...
		ps.inline_breakable_of(BreakableDelims::for_brace_block(), |ps| {
			...
			if let Some(body) = block_node.body() {
				...
				if has_multiple_statements {
					...
					ps.with_start_of_line(false, |ps| {
						let statements =
							body.as_statements_node().unwrap().body();
...
fn format_block_parameters_node<'src>(
	...
	if block_parameters_node.locals().is_empty()
...
fn format_array_node<'src>(
	ps: &mut ParserState<'src>,
	array_node: prism::ArrayNode<'src>,
) {
		...
		|ps| {
			for (idx, expr) in node_list.iter().enumerate() {
				...
				if let Some(string_node) = expr.as_string_node() {
					...
					let content = loc_to_str(string_node.content_loc());
					let escaped =
						crate::string_escape::escape_word_array_content(
							content,
							orig_open_delim,
							orig_close_delim,
						);
				...
				} else if let Some(interpolated_symbol_node) =
					expr.as_interpolated_symbol_node()
				{
					ps.at_offset(
						interpolated_symbol_node.location().start_offset(),
					);
				...
				} else if let Some(interpolated_string_node) =
					expr.as_interpolated_string_node()
				{
					ps.at_offset(
						interpolated_string_node.location().start_offset(),
					);
					format_word_array_interpolated_parts(
						ps,
					...
					if cfg!(debug_assertions) {
						unreachable!(
							"Received unexpected node in word array: {:?}",
							expr
						);
					}
					format_node(ps, expr);
...
fn format_word_array_interpolated_parts<'src>(
	...
	for part in parts.iter() {
		...
		if let Some(string_node) = part.as_string_node() {
			...
			ps.emit_string_content(escaped);
		} else if let Some(embedded_statements_node) =
			part.as_embedded_statements_node()
		{
...
fn split_node_into_call_chains<'src>(
	node: prism::Node<'src>,
) -> Vec<Vec<prism::Node<'src>>> {
...
fn format_splat_node<'src>(
	ps: &mut ParserState<'src>,
	splat_node: prism::SplatNode<'src>,
) {
	ps.emit_ident("*");
...
fn format_integer_node<'src>(
	ps: &mut ParserState<'src>,
	integer_node: prism::IntegerNode<'src>,
) {
	handle_string_at_offset(
		ps,
...
fn format_float_node<'src>(
	ps: &mut ParserState<'src>,
	float_node: prism::FloatNode<'src>,
) {
	handle_string_at_offset(
		ps,
...
fn format_for_node<'src>(
	ps: &mut ParserState<'src>,
	for_node: prism::ForNode<'src>,
) {
	ps.emit_keyword("for");
	ps.emit_space();
...
fn format_super_node<'src>(
	ps: &mut ParserState<'src>,
	super_node: prism::SuperNode<'src>,
) {
	ps.emit_ident("super");
	...
	ps.with_start_of_line(false, |ps| {
		ps.breakable_of(BreakableDelims::for_method_call(), |ps| {
				...
				&& let Some(block_arg) = block.as_block_argument_node()
			{
				...
				ps.with_start_of_line(false, |ps| {
					format_block_argument_node(ps, block_arg)
				});
			}
		});
			...
			&& let Some(block_node) = block.as_block_node()
		{
			ps.emit_space();
			ps.with_start_of_line(false, |ps| {
				format_block_node(ps, block_node)
			});
		}
	});
}
...
fn format_hash_node<'src>(
	ps: &mut ParserState<'src>,
	hash_node: prism::HashNode<'src>,
) {
	ps.with_start_of_line(false, |ps| {
		if hash_node.elements().is_empty() {
			...
			let has_comments =
				ps.has_comment_in_offset_span(start_offset, end_offset);
...
impl<'pr> Conditional<'pr> {
	...
	fn subsequent_or_else(&self) -> Option<prism::Node<'pr>> {
		match self {
			...
			Conditional::Unless(node) => {
				node.else_clause().map(|ec| ec.as_node())
			}
...
fn format_conditional_node<'src>(
	...
	let is_modifier = if let Some(statements) = conditional.statements() {
		statements.location().start_offset()
			< conditional.keyword_start_offset()
	} else {
		false
	};
	...
	if is_modifier {
		...
		let should_convert_to_block = match conditional {
			...
			Conditional::If(_) | Conditional::Unless(_) => {
				...
				let predicate_start_line = ps.get_line_number_for_offset(
					predicate.location().start_offset(),
				);
				let predicate_end_line = ps.get_line_number_for_offset(
...
fn format_if_node<'src>(
	ps: &mut ParserState<'src>,
	if_node: prism::IfNode<'src>,
) {
			...
			ps.with_start_of_line(false, |ps| {
				...
				format_node(
					...
					if_node
						...
						.first()
						.expect(
							"There must be exactly one statement inside a ternary branch",
						),
				);
				format_node(
					ps,
...
fn format_in_node<'src>(
	ps: &mut ParserState<'src>,
	in_node: prism::InNode<'src>,
) {
	ps.emit_in_keyword();
...
fn format_index_and_write_node<'src>(
	...
	ps.with_start_of_line(false, |ps| {
		format_node(ps, index_and_write_node.value())
	});
}
...
fn format_index_or_write_node<'src>(
	...
	ps.with_start_of_line(false, |ps| {
		format_node(ps, index_or_write_node.value())
	});
}
...
fn format_index_target_node<'src>(
	...
	ps.with_start_of_line(false, |ps| {
		format_node(ps, index_target_node.receiver())
	});
...
fn format_lambda_node<'src>(
	ps: &mut ParserState<'src>,
	lambda_node: prism::LambdaNode<'src>,
) {
	...
	ps.with_start_of_line(false, |ps| {
		...
		if let Some(parameters_node) = lambda_node.parameters() {
			if operator == "->"
				&& let Some(block_parameters) =
					parameters_node.as_block_parameters_node()
			{
				if block_parameters.parameters().is_some()
					|| !block_parameters.locals().is_empty()
				{
					ps.emit_space();
					ps.breakable_of(BreakableDelims::for_method_call(), |ps| {
						format_block_parameters_names(
...
fn format_multi_write_node<'src>(
	...
	ps.with_start_of_line(false, |ps| {
		format_node(ps, multi_write_node.value())
	});
}

fn format_next_node<'src>(
	ps: &mut ParserState<'src>,
	next_node: prism::NextNode<'src>,
) {
	ps.emit_ident("next");
...
fn format_or_node<'src>(
	ps: &mut ParserState<'src>,
	or_node: prism::OrNode<'src>,
) {
	ps.inline_breakable_of(BreakableDelims::for_binary_op(), |ps| {
		ps.with_start_of_line(false, |ps| {
			format_infix_operator(
...
fn format_range_node<'src>(
	ps: &mut ParserState<'src>,
	range_node: prism::RangeNode<'src>,
) {
	ps.with_start_of_line(false, |ps| {
...
fn format_rescue_node<'src>(
	ps: &mut ParserState<'src>,
	rescue_node: prism::RescueNode<'src>,
) {
	// Double check that these offsets are correct, since begin/rescue/ensure/else
	// aren't always handled with `format_node`, which usually handles this
...
fn format_return_node<'src>(
	ps: &mut ParserState<'src>,
	return_node: prism::ReturnNode<'src>,
) {
...
fn format_true_node<'src>(
	ps: &mut ParserState<'src>,
	true_node: prism::TrueNode<'src>,
) {
	handle_string_at_offset(ps, "true", true_node.location().start_offset());
}

fn format_undef_node<'src>(
	ps: &mut ParserState<'src>,
	undef_node: prism::UndefNode<'src>,
) {
...
fn format_unless_node<'src>(
	ps: &mut ParserState<'src>,
	unless_node: prism::UnlessNode<'src>,
) {
	format_conditional_node(
		ps,
		"unless",
		true,
		&Conditional::Unless(unless_node),
	);
}

fn format_until_node<'src>(
	ps: &mut ParserState<'src>,
	until_node: prism::UntilNode<'src>,
) {
	format_conditional_node(ps, "until", true, &Conditional::Until(until_node));
}

fn format_when_node<'src>(
	ps: &mut ParserState<'src>,
	when_node: prism::WhenNode<'src>,
) {
...
fn format_while_node<'src>(
	ps: &mut ParserState<'src>,
	while_node: prism::WhileNode<'src>,
) {
	format_conditional_node(ps, "while", true, &Conditional::While(while_node));
}

fn format_x_string_node<'src>(
	ps: &mut ParserState<'src>,
	x_string_node: prism::XStringNode<'src>,
) {
	ps.emit_ident("`");
...
fn format_yield_node<'src>(
	ps: &mut ParserState<'src>,
	yield_node: prism::YieldNode<'src>,
) {
	ps.emit_ident("yield");
...
fn unwrap_single_arg_paren<'src>(
	node: &prism::Node<'src>,
) -> Option<prism::Node<'src>> {
			...
			for (idx, expr) in node_list.iter().enumerate() {
				if single_line {
					...
					ps.with_start_of_line(false, |ps| {
						...
						} else if let Some(splat_node) =
							expr.as_assoc_splat_node()
						{
							if idx > 0 {
								ps.emit_soft_indent();
							}
