...
impl FileComments {
	pub fn from_prism_comments(
		...
		for comment in comments {
			file_comments.push_comment(
				line_index.get_line_number(comment.location().start_offset())
					as u64,
	...
	fn line_has_ruby(line: &[u8], inside_embdoc: &mut bool) -> bool {
		let first_non_ws =
			line.iter().position(|b| !u8::is_ascii_whitespace(b));
		...
		match (
			...
			_ => {
				debug_assert!(
					self.other_comments
						.last()
						.map(|(last_line_number, _)| *last_line_number
							< line_number)
						.unwrap_or(true),
	...
	pub fn take_start_of_file_contiguous_comment_lines(
		&mut self,
	) -> Option<CommentBlock> {
		self.start_of_file_contiguous_comment_lines.take()
	}

	pub fn has_comments_in_lines(
		&self,
	...
	pub fn extract_comments_to_line(
		...
		for (index, comment_contents) in
			self.other_comments.drain(..split_point)
		{
		...
		Some((
			CommentBlock::new(
				lowest_line..line_number + 1,
				comment_block_with_spaces,
			),
			last_line.unwrap(),
		))
	}
