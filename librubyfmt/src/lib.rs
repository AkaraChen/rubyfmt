...
pub fn format_buffer(buf: &str) -> Result<String, RichFormatError> {
	...
	Ok(String::from_utf8(output.into_inner())
		.expect("we never write invalid UTF-8"))
}
...
pub fn toplevel_format_program_with_prism<W: Write>(
	...
	let mut ps =
		ParserState::new(FileComments::from_prism_comments(comments, source));
	ps.flush_start_of_file_comments();

	format_prism::format_program(
		&mut ps,
		tree.as_program_node().unwrap(),
		data,
	);
