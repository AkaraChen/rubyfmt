...
impl<'src> RenderQueueWriter<'src> {
	...
	fn render_as(
		...
		while let Some((index, mut next_token)) = peekable.next() {
			...
			match &next_token {
				ConcreteLineTokenAndTargets::ConcreteLineToken(Indent {
					depth,
				}) => {
					...
					if !is_ending_heredoc_token {
						next_token =
							clats_indent(depth + accum.additional_indent)
					}
				}
				ConcreteLineTokenAndTargets::ConcreteLineToken(
				...
				match x {
					...
					_ => {
						if x.is_in_need_of_a_trailing_blankline()
							...
							accum.insert_blankline_from_end(
	...
	fn format_breakable_entry(
		accum: &mut Intermediary<'src>,
		be: BreakableEntry<'src>,
	) {
		...
		let force_single_line = !be
			.any_collapsing_newline_has_heredoc_content()
			&& be.in_string_embexpr();

		let mut tokens = Vec::new();
		if !force_single_line
			&& (be.is_multiline()
				|| Self::renders_over_max_line_length(accum, &be))
		{
	...
	fn format_breakable_call_chain_entry(
		...
		if must_multiline
			|| ((bcce.is_multiline()
				|| Self::renders_over_max_line_length(accum, &bcce))
				&& !bcce.in_string_embexpr())
		{
	...
	fn renders_over_max_line_length(
		...
		breakable.single_line_string_length(accum.current_line_length())
			> MAX_LINE_LENGTH
	}
