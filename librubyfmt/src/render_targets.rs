...
use crate::line_tokens::{
	AbstractLineToken, ConcreteLineToken, ConcreteLineTokenAndTargets,
};
...
impl<'src> BreakableEntry<'src> {
	...
	pub fn single_line_len_params_only(&self) -> usize {
		if let Some(AbstractLineToken::BreakableEntry(params)) =
			self.tokens.first()
...
impl<'src> AbstractTokenTarget<'src> for BreakableCallChainEntry<'src> {
	...
	fn single_line_string_length(&self, current_line_length: usize) -> usize {
		...
		if let Some((AbstractLineToken::BreakableEntry(_), rest)) =
			tokens.split_first()
		{
		...
		if let Some((
			AbstractLineToken::ConcreteLineToken(
				ConcreteLineToken::EndCallChainIndent,
			),
			rest,
		)) = tokens.split_last()
		{
		...
		if let Some((AbstractLineToken::BreakableEntry(be), rest)) =
			tokens.split_last()
...
impl<'src> Breakable<'src> {
	...
	pub fn into_breakable_call_chain(
		self,
	) -> Option<BreakableCallChainEntry<'src>> {
		match self {
