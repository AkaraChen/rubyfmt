...
pub fn clats_direct_part<'src>(
	...
	ConcreteLineTokenAndTargets::ConcreteLineToken(
		ConcreteLineToken::DirectPart { part: part.into() },
	)
}

pub fn clats_heredoc_close<'src>(
	symbol: String,
) -> ConcreteLineTokenAndTargets<'src> {
	ConcreteLineTokenAndTargets::ConcreteLineToken(
...
impl<'src> ConcreteLineToken<'src> {
	pub fn into_ruby(self) -> Cow<'src, str> {
		match self {
			...
			| Self::EndCallChainIndent => Cow::Borrowed(""),
		}
	}
	...
	pub fn len(&self) -> usize {
		...
		match self {
			...
			Keyword { keyword: contents } | ConditionalKeyword { contents } => {
				contents.len()
			}
			...
			HardNewLine | Comma | Space | Dot | OpenSquareBracket
			| CloseSquareBracket | OpenCurlyBracket | CloseCurlyBracket
			| OpenParen | CloseParen | SingleSlash | DoubleQuote => 1,
...
impl<'src> From<ConcreteLineTokenAndTargets<'src>> for AbstractLineToken<'src> {
	fn from(
		cltat: ConcreteLineTokenAndTargets<'src>,
	) -> AbstractLineToken<'src> {
		match cltat {
	...
	pub fn write_single_line(
		...
		match self {
			...
			Self::BreakableCallChainEntry(bcce) => {
				out.push(ConcreteLineTokenAndTargets::BreakableCallChainEntry(
					bcce,
				));
			}
		}
	...
	fn write_heredocs(
		...
		if let Some(values) = heredoc_strings {
			for hds in values {
				...
				out.push(cltats_hard_newline());
				let indent_depth =
					if indent != 0 { indent - 2 } else { indent };
